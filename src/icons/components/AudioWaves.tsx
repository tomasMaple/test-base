import type { SVGProps } from "react";
const SvgAudioWaves = (props: SVGProps<SVGSVGElement>) => (
  <svg
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="1em"
    height="1em"
    aria-hidden="true"
    {...props}
  >
    <path
      d="M12 3C12.5523 3 13 3.44772 13 4V20C13 20.5523 12.5523 21 12 21C11.4477 21 11 20.5523 11 20V4C11 3.44772 11.4477 3 12 3Z"
      fill="currentColor"
    />
    <path
      d="M16 6.5C16.5523 6.5 17 6.94772 17 7.5V16.5C17 17.0523 16.5523 17.5 16 17.5C15.4477 17.5 15 17.0523 15 16.5V7.5C15 6.94772 15.4477 6.5 16 6.5Z"
      fill="currentColor"
    />
    <path
      d="M21 10C21 9.44772 20.5523 9 20 9C19.4477 9 19 9.44772 19 10V14C19 14.5523 19.4477 15 20 15C20.5523 15 21 14.5523 21 14V10Z"
      fill="currentColor"
    />
    <path
      d="M8 6.5C8.55228 6.5 9 6.94772 9 7.5V16.5C9 17.0523 8.55228 17.5 8 17.5C7.44772 17.5 7 17.0523 7 16.5V7.5C7 6.94772 7.44772 6.5 8 6.5Z"
      fill="currentColor"
    />
    <path
      d="M5 10C5 9.44772 4.55228 9 4 9C3.44772 9 3 9.44772 3 10V14C3 14.5523 3.44772 15 4 15C4.55228 15 5 14.5523 5 14V10Z"
      fill="currentColor"
    />
  </svg>
);
export default SvgAudioWaves;
