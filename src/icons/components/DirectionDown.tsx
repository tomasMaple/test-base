import type { SVGProps } from "react";
const SvgDirectionDown = (props: SVGProps<SVGSVGElement>) => (
  <svg
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="1em"
    height="1em"
    aria-hidden="true"
    {...props}
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M18.7071 12.2929C18.3166 11.9024 17.6834 11.9024 17.2929 12.2929L12 17.5858L6.70711 12.2929C6.31658 11.9024 5.68342 11.9024 5.29289 12.2929C4.90237 12.6834 4.90237 13.3166 5.29289 13.7071L11.2929 19.7071C11.4804 19.8946 11.7348 20 12 20C12.2652 20 12.5196 19.8946 12.7071 19.7071L18.7071 13.7071C19.0976 13.3166 19.0976 12.6834 18.7071 12.2929Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12 20C12.5523 20 13 19.5523 13 19V5C13 4.44771 12.5523 4 12 4C11.4477 4 11 4.44771 11 5V19C11 19.5523 11.4477 20 12 20Z"
      fill="currentColor"
    />
  </svg>
);
export default SvgDirectionDown;
