import type { SVGProps } from "react";
const SvgTextAlignLeft = (props: SVGProps<SVGSVGElement>) => (
  <svg
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="1em"
    height="1em"
    aria-hidden="true"
    {...props}
  >
    <path
      d="M4 5C3.44772 5 3 5.44772 3 6C3 6.55228 3.44772 7 4 7H20C20.5523 7 21 6.55228 21 6C21 5.44772 20.5523 5 20 5H4Z"
      fill="currentColor"
    />
    <path
      d="M3 10C3 9.44772 3.44772 9 4 9H15C15.5523 9 16 9.44772 16 10C16 10.5523 15.5523 11 15 11H4C3.44772 11 3 10.5523 3 10Z"
      fill="currentColor"
    />
    <path
      d="M4 13C3.44772 13 3 13.4477 3 14C3 14.5523 3.44772 15 4 15H20C20.5523 15 21 14.5523 21 14C21 13.4477 20.5523 13 20 13H4Z"
      fill="currentColor"
    />
    <path
      d="M4 17C3.44772 17 3 17.4477 3 18C3 18.5523 3.44772 19 4 19H15C15.5523 19 16 18.5523 16 18C16 17.4477 15.5523 17 15 17H4Z"
      fill="currentColor"
    />
  </svg>
);
export default SvgTextAlignLeft;
