import type { SVGProps } from "react";
const SvgPetPaw = (props: SVGProps<SVGSVGElement>) => (
  <svg
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="1em"
    height="1em"
    aria-hidden="true"
    {...props}
  >
    <path
      d="M7 5C7 3.89543 7.89543 3 9 3C10.1046 3 11 3.89543 11 5C11 6.10457 10.1046 7 9 7C7.89543 7 7 6.10457 7 5Z"
      fill="currentColor"
    />
    <path
      d="M3 9C3 7.89543 3.89543 7 5 7C6.10457 7 7 7.89543 7 9C7 10.1046 6.10457 11 5 11C3.89543 11 3 10.1046 3 9Z"
      fill="currentColor"
    />
    <path
      d="M15 3C13.8954 3 13 3.89543 13 5C13 6.10457 13.8954 7 15 7C16.1046 7 17 6.10457 17 5C17 3.89543 16.1046 3 15 3Z"
      fill="currentColor"
    />
    <path
      d="M17 9C17 7.89543 17.8954 7 19 7C20.1046 7 21 7.89543 21 9C21 10.1046 20.1046 11 19 11C17.8954 11 17 10.1046 17 9Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M19 15.5C19 12.46 15.865 9 12 9C8.135 9 5 12.46 5 15.5C5 18.54 8.135 21 12 21C15.865 21 19 19.5 19 15.5ZM12 11C13.326 11 14.5917 11.5972 15.5475 12.5365C16.5257 13.4977 17 14.6352 17 15.5C17 16.8932 16.4989 17.6304 15.8128 18.1044C15.0124 18.6572 13.7243 19 12 19C8.76256 19 7 17.0123 7 15.5C7 14.6352 7.47433 13.4977 8.45247 12.5365C9.40827 11.5972 10.674 11 12 11Z"
      fill="currentColor"
    />
  </svg>
);
export default SvgPetPaw;
