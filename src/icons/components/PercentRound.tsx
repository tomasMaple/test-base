import type { SVGProps } from "react";
const SvgPercentRound = (props: SVGProps<SVGSVGElement>) => (
  <svg
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="1em"
    height="1em"
    aria-hidden="true"
    {...props}
  >
    <path
      d="M9 7.5C8.17157 7.5 7.5 8.17157 7.5 9C7.5 9.82843 8.17167 10.5 9.0001 10.5C9.82853 10.5 10.5001 9.82843 10.5001 9C10.5001 8.17157 9.82843 7.5 9 7.5Z"
      fill="currentColor"
    />
    <path
      d="M15 13.5C14.1716 13.5 13.5 14.1716 13.5 15C13.5 15.8284 14.1717 16.5 15.0001 16.5C15.8285 16.5 16.5001 15.8284 16.5001 15C16.5001 14.1716 15.8284 13.5 15 13.5Z"
      fill="currentColor"
    />
    <path
      d="M15.7071 8.29289C16.0976 8.68342 16.0976 9.31658 15.7071 9.70711L9.70711 15.7071C9.31658 16.0976 8.68342 16.0976 8.29289 15.7071C7.90237 15.3166 7.90237 14.6834 8.29289 14.2929L14.2929 8.29289C14.6834 7.90237 15.3166 7.90237 15.7071 8.29289Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2ZM4 12C4 7.58172 7.58172 4 12 4C16.4183 4 20 7.58172 20 12C20 16.4183 16.4183 20 12 20C7.58172 20 4 16.4183 4 12Z"
      fill="currentColor"
    />
  </svg>
);
export default SvgPercentRound;
