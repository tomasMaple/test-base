import type { SVGProps } from "react";
const SvgPetPawFill = (props: SVGProps<SVGSVGElement>) => (
  <svg
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="1em"
    height="1em"
    aria-hidden="true"
    {...props}
  >
    <path
      d="M9 3C7.89543 3 7 3.89543 7 5C7 6.10457 7.89543 7 9 7C10.1046 7 11 6.10457 11 5C11 3.89543 10.1046 3 9 3Z"
      fill="currentColor"
    />
    <path
      d="M5 7C3.89543 7 3 7.89543 3 9C3 10.1046 3.89543 11 5 11C6.10457 11 7 10.1046 7 9C7 7.89543 6.10457 7 5 7Z"
      fill="currentColor"
    />
    <path
      d="M15 3C13.8954 3 13 3.89543 13 5C13 6.10457 13.8954 7 15 7C16.1046 7 17 6.10457 17 5C17 3.89543 16.1046 3 15 3Z"
      fill="currentColor"
    />
    <path
      d="M19 7C17.8954 7 17 7.89543 17 9C17 10.1046 17.8954 11 19 11C20.1046 11 21 10.1046 21 9C21 7.89543 20.1046 7 19 7Z"
      fill="currentColor"
    />
    <path
      d="M19 15.5C19 12.46 15.865 9 12 9C8.135 9 5 12.46 5 15.5C5 18.54 8.135 21 12 21C15.865 21 19 19.5 19 15.5Z"
      fill="currentColor"
    />
  </svg>
);
export default SvgPetPawFill;
