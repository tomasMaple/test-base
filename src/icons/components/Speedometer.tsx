import type { SVGProps } from "react";
const SvgSpeedometer = (props: SVGProps<SVGSVGElement>) => (
  <svg
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width="1em"
    height="1em"
    aria-hidden="true"
    {...props}
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20C16.4183 20 20 16.4183 20 12ZM12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12 2.5C12.5523 2.5 13 2.94772 13 3.5V6C13 6.55228 12.5523 7 12 7C11.4477 7 11 6.55228 11 6V3.5C11 2.94772 11.4477 2.5 12 2.5Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12 13C12.5523 13 13 13.4477 13 14V17C13 17.5523 12.5523 18 12 18C11.4477 18 11 17.5523 11 17V14C11 13.4477 11.4477 13 12 13Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M4.79289 5.29289C5.18342 4.90237 5.81658 4.90237 6.20711 5.29289L7.97713 7.06291C8.36765 7.45344 8.36765 8.0866 7.97713 8.47713C7.5866 8.86765 6.95344 8.86765 6.56291 8.47713L4.79289 6.70711C4.40237 6.31658 4.40237 5.68342 4.79289 5.29289Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M2 12C2 11.4477 2.44772 11 3 11H6C6.55229 11 7 11.4477 7 12C7 12.5523 6.55229 13 6 13H3C2.44772 13 2 12.5523 2 12Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M17 12C17 11.4477 17.4477 11 18 11H21C21.5523 11 22 11.4477 22 12C22 12.5523 21.5523 13 21 13H18C17.4477 13 17 12.5523 17 12Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M19.2076 5.29289C19.5981 5.68342 19.5981 6.31658 19.2076 6.70711L17.4376 8.47713C17.0471 8.86765 16.4139 8.86765 16.0234 8.47713C15.6328 8.0866 15.6328 7.45344 16.0234 7.06291L17.7934 5.29289C18.1839 4.90237 18.8171 4.90237 19.2076 5.29289Z"
      fill="currentColor"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13C12.5523 13 13 12.5523 13 12C13 11.4477 12.5523 11 12 11ZM9 12C9 10.3431 10.3431 9 12 9C13.6569 9 15 10.3431 15 12C15 13.6569 13.6569 15 12 15C10.3431 15 9 13.6569 9 12Z"
      fill="currentColor"
    />
  </svg>
);
export default SvgSpeedometer;
